-- Area22 Music Request System - Supabase Database Setup
...
CREATE INDEX IF NOT EXISTS idx_events_dj_name ON events(dj_name);
...
CREATE INDEX IF NOT EXISTS idx_song_requests_guest_name ON song_requests(guest_name);
CREATE INDEX IF NOT EXISTS idx_song_requests_created_at ON song_requests(created_at);

-- Event settings table indexes
CREATE INDEX IF NOT EXISTS idx_event_settings_session_id ON event_settings(session_id);

-- Guest sessions table indexes
CREATE INDEX IF NOT EXISTS idx_guest_sessions_session_id ON guest_sessions(session_id);
CREATE INDEX IF NOT EXISTS idx_guest_sessions_guest_name ON guest_sessions(guest_name);
...
CREATE INDEX IF NOT EXISTS idx_event_analytics_session_id ON event_analytics(session_id);
...
RETURNS INTEGER AS $$
BEGIN
    RETURN (SELECT COALESCE(MAX(column_value), 0) + 1
            FROM (
                SELECT 
                    CASE column_name
                        WHEN 'total_requests' THEN total_requests
                        WHEN 'pending_requests' THEN pending_requests
                        WHEN 'completed_requests' THEN completed_requests
                        WHEN 'skipped_requests' THEN skipped_requests
                        WHEN 'unique_guests' THEN unique_guests
                        ELSE 0
                    END as column_value
                FROM event_analytics 
                WHERE session_id = row_id
            ) subquery);
END;
$$ LANGUAGE plpgsql;

-- Function to decrement a column value
CREATE OR REPLACE FUNCTION decrement(row_id TEXT, column_name TEXT)
RETURNS INTEGER AS $$
BEGIN
    RETURN (SELECT GREATEST(COALESCE(MAX(column_value), 0) - 1, 0)
            FROM (
                SELECT 
                    CASE column_name
                        WHEN 'total_requests' THEN total_requests
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        IF OLD.status != NEW.status THEN
            ...
            IF OLD.status = 'pending' THEN
                ...
                WHERE session_id = NEW.session_id;
            ELSIF OLD.status = 'playing' THEN
                -- No specific counter for playing, handled in status change
            ...
            IF NEW.status = 'playing' THEN
                -- No specific counter for playing
...
CREATE POLICY "Anyone can insert requests for active events" ON song_requests
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM events 
            WHERE session_id = song_requests.session_id 
            AND status = 'active'
        )
    );

CREATE POLICY "Anyone can view requests for active events" ON song_requests
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM events 
            WHERE session_id = song_requests.session_id 
...
CREATE POLICY "Authenticated users can update/delete requests" ON song_requests
    FOR ALL USING (auth.role() = 'authenticated');

-- Event settings table policies
CREATE POLICY "Anyone can view settings for active events" ON event_settings
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM events 
            WHERE session_id = event_settings.session_id 
            AND status = 'active'
        )
    );

CREATE POLICY "Authenticated users can manage settings" ON event_settings
    FOR ALL USING (auth.role() = 'authenticated');
...
CREATE POLICY "Authenticated users can view all guest sessions" ON guest_sessions
    FOR SELECT USING (auth.role() = 'authenticated');

-- DJ profiles table policies
CREATE POLICY "Users can view their own profile" ON dj_profiles
    FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Users can update their own profile" ON dj_profiles
    FOR UPDATE USING (user_id = auth.uid());

CREATE POLICY "Users can insert their own profile" ON dj_profiles
    FOR INSERT WITH CHECK (user_id = auth.uid());

-- Event analytics table policies
CREATE POLICY "Anyone can view analytics for active events" ON event_analytics
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM events 
            WHERE session_id = event_analytics.session_id 
            AND status = 'active'
        )
    );

CREATE POLICY "System can update analytics" ON event_analytics
...
-- Insert sample event settings
INSERT INTO event_settings (session_id, allow_anonymous, require_approval, max_requests_per_guest, custom_message, theme_color)
VALUES ('DEMO-20240615-ABC', true, false, 5, 'Welcome to our demo event!', '#00ff00')
ON CONFLICT (session_id) DO NOTHING;

-- Insert sample analytics
INSERT INTO event_analytics (session_id, total_requests, pending_requests, completed_requests, skipped_requests, unique_guests)
VALUES ('DEMO-20240615-ABC', 0, 0, 0, 0, 0)
ON CONFLICT (session_id) DO NOTHING;
...
AND routine_name IN ('update_updated_at_column', 'increment', 'decrement', 'create_event_related_records', 'update_analytics_on_request_change')
